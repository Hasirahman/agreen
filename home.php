<!-- Slider Revolution -->
<div class="revolution-slider-container">
	<div class="revolution-slider" data-version="5.4.8" style="display: none;">
		<ul style="display: none;">
			<!-- SLIDE 1 -->
			<li data-transition="fade" data-masterspeed="500" data-slotamount="1" data-delay="6000">
				<!-- MAIN IMAGE -->
				<img src="images/slider/image_011.jpg" alt="slidebg1" data-bgfit="cover">
				<!-- LAYERS -->
				<!-- LAYER 01 -->
				<div class="tp-caption"
					data-frames='[{"delay":500,"speed":1200,"from":"x:40;o:0;","ease":"easeInOutExpo"},{"delay":"wait","speed":500,"to":"o:0;","ease":"easeInOutExpo"}]'
					data-x="0"
					data-y="140"
					>
					<!--data-start="500"
					data-transform_in="x:40;y:0;o:0;s:1200;e:easeInOutExpo;"
					data-transform_out="x:0;y:0;o:0;s:500;e:easeInOutExpo;"-->
					
					<!--<div class="slider-content-box">
						<h2><a href="?page=service_interior_renovation" title="INTERIOR RENOVATION">HOME IMPROVEMENTS AND REMODELING</a></h2>
						<p>With over 15 years experience and real focus on customer satisfaction, you can rely on us for your next renovation, remodeling or driveway sett.</p>
						<a target="_top" class="more simple" href="https://1.envato.market/c/296919/275988/4415?u=https://themeforest.net/cart/add_items?item_ids=11313006&amp;ref=QuanticaLabs" title="BUY THIS TEMPLATE">BUY THIS TEMPLATE</a>
					</div>-->
				</div>
			</li>

			<!-- SLIDE 2 -->
			<li data-transition="fade" data-masterspeed="500" data-slotamount="1" data-delay="6000">
				<!-- MAIN IMAGE -->
				<img src="images/slider/image_022.jpg" alt="slidebg2" data-bgfit="cover">
				<!-- LAYERS -->
				<!-- LAYER 01 -->
				<div class="tp-caption"
					data-frames='[{"delay":500,"speed":1200,"from":"x:40;o:0;","ease":"easeInOutExpo"},{"delay":"wait","speed":500,"to":"o:0;","ease":"easeInOutExpo"}]'
					data-x="0"
					data-y="140"
					>
					<!--data-start="500"
					data-transform_in="x:40;y:0;o:0;s:1200;e:easeInOutExpo;"
					data-transform_out="x:0;y:0;o:0;s:500;e:easeInOutExpo;"-->
					
					<!--<div class="slider-content-box">
						<h2><a href="?page=service_tiling_painting" title="TILING AND PAINTING">PROFESSIONAL TILING AND PAINTING SERVICES</a></h2>
						<p>We combine quality workmanship, superior knowledge and low prices to provide you with service unmatched by our competitors.</p>
						<a target="_top" class="more simple" href="https://1.envato.market/c/296919/275988/4415?u=https://themeforest.net/cart/add_items?item_ids=11313006&amp;ref=QuanticaLabs" title="BUY THIS TEMPLATE">BUY THIS TEMPLATE</a>
					</div>-->
				</div>
			</li>
			
			<!-- SLIDE 3 -->
			<li data-transition="fade" data-masterspeed="500" data-slotamount="1" data-delay="6000">
				<!-- MAIN IMAGE -->
				<img src="images/slider/image_033.jpg" alt="slidebg3" data-bgfit="cover">
				<!-- LAYERS -->
				<!-- LAYER 01 -->
				<div class="tp-caption"
					data-frames='[{"delay":500,"speed":1200,"from":"x:40;o:0;","ease":"easeInOutExpo"},{"delay":"wait","speed":500,"to":"o:0;","ease":"easeInOutExpo"}]'
					data-x="0"
					data-y="140"
					>
					<!--data-start="500"
					data-transform_in="x:40;y:0;o:0;s:1200;e:easeInOutExpo;"
					data-transform_out="x:0;y:0;o:0;s:500;e:easeInOutExpo;"-->
					
					<!--<div class="slider-content-box">
						<h2><a href="?page=service_household_repairs" title="HOUSEHOLD REPAIRS">YOUR MOST AFFORDABLE CONTRACTORS</a></h2>
						<p>We have the experience, personel and resources to make the project run smoothly. We can ensure a job is done on time.</p>
						<a target="_top" class="more simple" href="https://1.envato.market/c/296919/275988/4415?u=https://themeforest.net/cart/add_items?item_ids=11313006&amp;ref=QuanticaLabs" title="BUY THIS TEMPLATE">BUY THIS TEMPLATE</a>
					</div>-->
				</div>
			</li>
		</ul>
	</div>
</div>
<!--/-->

<div class="theme-page">
	<div class="clearfix">
		 
		<div class="row page-margin-top-section padding-bottom-61">
			<div class="row">
				<h2 class="box-header"> AG Engineering Services</h2>
				<p class="description align-center">The organization was established in 2022, and we have over 2 years of rich experience in the air-molding industry.  Our comprehensive services include customized design, AC repair and maintenance, installation, tailored preventive maintenance contracts, and support for our clients across various industries. These include apartments, luxury villas, office spaces, vehicle showrooms, IT companies, manufacturing units, resorts, hospitals, and more.</p>
			</div>
			<div class="row page-margin-top ">
				<div class="column column-1-2 align-center re-preload">
					<div class="image-wrapper">
						<img src="images/samples/480x480/image_11.jpg" alt="" class="radius">
					</div>
				</div>
				<div class="column column-1-2">
					<ul class="features-list">
						<li class="sl-small-helmet margin-top-40">
							<h4>OVER 2 YEARS EXPERIENCE</h4>
							<p>We have a design team, sales team, and service dealers for multiple brands in HVAC & R, AC repair, and AC services</p>
						</li>
						<li class="sl-small-roller">
							<h4>BEST MATERIALS</h4>
							<p>We have the experience, personel and resources to make the project run smoothly. We can ensure a job is done on time.</p>
						</li>
						<li class="sl-small-driller">
							<h4>PROFESSIONAL STANDARDS</h4>
							<p>Work with us involve a carefully planned series of steps, centered around a schedule we stick to and daily communication.</p>
						</li>
					</ul>
				</div>
			</div>
		</div>
		<div class="row full-width gray page-padding-top-section">
			<div class="row">
				<h2 class="box-header">OUR SERVICES</h2> 
				<ul class="services-list clearfix page-margin-top">
					<li>
						<a href="?page=service_interior_renovation" title="Interior Renovation">
							<img src="images/samples/390x260/image_11.jpg" alt="">
						</a>
						<h4 class="box-header"><a href="?page=service_interior_renovation" title="Interior Renovation">Designing & Drafting</a></h4>
						 
					</li>
					<li>
						<a href="?page=service_design_build" title="Design and Build">
							<img src="images/samples/390x260/image_12.jpg" alt="">
						</a>
						<h4 class="box-header"><a href="?page=service_design_build" title="Design and Build">Installations</a></h4>
						 
					</li>
					<li>
						<a href="?page=service_tiling_painting" title="Tiling and Painting">
							<img src="images/samples/390x260/image_13.jpg" alt="">
						</a>
						<h4 class="box-header"><a href="?page=service_tiling_painting" title="Tiling and Painting">Repairs & Services</a></h4>
						 
					</li>
				</ul> 
			</div>
		</div>
		<div class="row full-width page-margin-top-section">
			<div class="row">
				<h2 class="box-header">OUR PRODUCTS</h2>
				<!--<p class="description align-center">Here are a few of many projects we have completed for our customers. We provide a professional service<br>which includes consultation, free estimate, design, supply of materials and installation.</p>-->
			</div>	
			<ul class="projects-list clearfix page-margin-top">
				<li>
					<a href="?page=project_design_build" title="Design and Build">
						<img src="images/samples/480x320/image_11.jpg" alt="">
					</a>
					<div class="view align-center">
						<div class="vertical-align-table">
							<div class="vertical-align-cell">
								<p class="description">Design and Build</p>
								<a class="more simple" href="?page=project_design_build" title="VIEW PRODUCTS">VIEW PRODUCTS</a>
							</div>
						</div>
					</div>
				</li>
				<li>
					<a href="?page=project_garden_renovation" title="Garden Renovation">
						<img src="images/samples/480x320/image_12.jpg" alt="">
					</a>
					<div class="view align-center">
						<div class="vertical-align-table">
							<div class="vertical-align-cell">
								<p class="description">Garden Renovation</p>
								<a class="more simple" href="?page=project_garden_renovation" title="VIEW PRODUCTS">VIEW PRODUCTS</a>
							</div>
						</div>
					</div>
				</li>
				<li>
					<a href="?page=project_interior_renovation" title="Interior Renovation">
						<img src="images/samples/480x320/image_13.jpg" alt="">
					</a>
					<div class="view align-center">
						<div class="vertical-align-table">
							<div class="vertical-align-cell">
								<p class="description">Interior Renovation</p>
								<a class="more simple" href="?page=project_interior_renovation" title="VIEW PRODUCTS">VIEW PRODUCTS</a>
							</div>
						</div>
					</div>
				</li>
				<li>
					<a href="?page=project_solar_systems" title="Solar Systems">
						<img src="images/samples/480x320/image_14.jpg" alt="">
					</a>
					<div class="view align-center">
						<div class="vertical-align-table">
							<div class="vertical-align-cell">
								<p class="description">Solar Systems</p>
								<a class="more simple" href="?page=project_solar_systems" title="VIEW PRODUCTS">VIEW PRODUCTS</a>
							</div>
						</div>
					</div>
				</li>
			</ul>
		</div>
		<div class="row gray full-width padding-top-54 padding-bottom-61 align-center">
			<!--<h3><span class="button-label">EXPLORE OUR PROJECTS</span> <a class="more" href="?page=projects" title="VIEW ALL PROJECTS">VIEW ALL PROJECTS</a></h3>-->
			<h3><span class="button-label">EXPLORE OUR PRODUCTS</span> <a class="more" href="#" title="VIEW ALL PROJECTS">VIEW ALL PRODUCTS</a></h3>

		</div>
		<!--<div class="row gray full-width padding-top-bottom-50">
			<div class="row">
				<div class="column column-1-2 align-right padding-top-7">
					<h3>EXPLORE OUR PROJECTS</h3>
				</div>
				<div class="column column-1-2">
					<a class="more float" href="?page=projects" title="VIEW ALL PROJECTS">VIEW ALL PROJECTS</a>
				</div>
			</div>
		</div>-->
		 
		 
		<div class="row full-width padding-top-70 padding-bottom-66 parallax parallax-1">	
			<div class="row testimonials-container">
				<a href="#" class="slider-control left template-arrow-left-1"></a>
				<ul class="testimonials-list">
					<li class="sl-small-conversation">
						<div class="ornament"></div>
						<p>"We would like to thank Renovate Company for an outstanding effort on this
						recently completed project located in the Moscow. The project involved a very
						aggressive schedule and it was completed on time. We would certainly like to
						use their professional services again."</p>
						<div class="author">MITCHEL SMITH</div>
						<div class="author-details">CEO OF NEW PORT COMPANY</div>
					</li>
					<li class="sl-small-conversation">
						<div class="ornament"></div>
						<p>"We would like to thank Renovate Company for an outstanding effort on this
						recently completed project located in the Moscow. The project involved a very
						aggressive schedule and it was completed on time. We would certainly like to
						use their professional services again."</p>
						<div class="author">MITCHEL SMITH</div>
						<div class="author-details">CEO OF NEW PORT COMPANY</div>
					</li>
					<li class="sl-small-conversation">
						<div class="ornament"></div>
						<p>"We would like to thank Renovate Company for an outstanding effort on this
						recently completed project located in the Moscow. The project involved a very
						aggressive schedule and it was completed on time. We would certainly like to
						use their professional services again."</p>
						<div class="author">MITCHEL SMITH</div>
						<div class="author-details">CEO OF NEW PORT COMPANY</div>
					</li>
				</ul>
				<a href="#" class="slider-control right template-arrow-left-1"></a>
			</div>
		</div>
		<div class="row page-margin-top-section padding-bottom-66">
			<h2 class="box-header">WHY CHOOSE US</h2>
			
			<div class="row page-margin-top">
				<div class="column column-1-4">
					<ul class="features-list big">
						<li class="sl-large-bubble-check">
							<div class="ornament"></div>
							<h4 class="box-header page-margin-top">STANDARDS</h4>
							<p>Consistently offer a product portfolio that differentiates us from others in the marketplace.</p>
						</li>
					</ul>
				</div>
				
				<div class="column column-1-4">
					<ul class="features-list big">
						<li class="sl-large-eco">
							<div class="ornament"></div>
							<h4 class="box-header page-margin-top">LONG-TEAM QUALITY</h4>
							<p>Provide complete transparency in our dealings with you. We guide you with a focus on long-term quality, efficiency, and clean air for your home, business, or industry</p>
						</li>
					</ul>
				</div>
				<div class="column column-1-4">
					<ul class="features-list big">
						<li class="sl-large-conversation">
							<div class="ornament"></div>
							<h4 class="box-header page-margin-top">QUALITY & RELIABILITY</h4>
							<p>Maintain an impeccable track record of quality and reliability, backed by an unmatched performance guarantee.</p>
						</li>
					</ul>
				</div>
				<div class="column column-1-4">
					<ul class="features-list big">
						<li class="sl-large-globe">
							<div class="ornament"></div>
							<h4 class="box-header page-margin-top">BEST PERFORMANCE</h4>
							<p>We offer our customers an unrivaled guarantee of performance and dependability, ensuring absolute peace of mind.</p>
						</li>
					</ul>
				</div>
			</div>

			<p class="description align-center">Finally, we aim to continue guaranteeing the performance of our products while bringing the latest AC products on the market within reach of our customers.</p>
		</div>
		
		

		<div class="row gray full-width page-padding-top-section padding-bottom-66">
			<div class="row">
				<h2 class="box-header">WE WORK WITH</h2>
				<div class="our-clients-list-container page-margin-top">
					<ul class="our-clients-list">
						<li>
							<img src="images/logos/logo_01.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_02.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_03.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_04.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_05.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_06.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_06.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_05.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_04.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_03.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_02.png" alt="">
						</li>
						<li>
							<img src="images/logos/logo_01.png" alt="">
						</li>
					</ul>
					<div class="re-carousel-pagination"></div>
				</div>
			</div>
		</div>
	</div>
</div>